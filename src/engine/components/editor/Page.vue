    ...
    >
      ...
      <v-dialog v-model="cameraDialog" persistent max-width="785px">
        <v-card>
            ...
            <v-container grid-list-md>
              <v-flex xs12>
                <video
                  ...
                  autoplay
                ></video>
                <v-btn flat class="btn-success" @click="capture">
                  Snap Photo
                </v-btn>
                <div
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn flat class="btn-danger" @click="closeCameraDialog">
              Save
            </v-btn>
          </v-card-actions>
        </v-card>
    ...
    Blockly.camera = cb => {
      ...
      if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
        navigator.mediaDevices.getUserMedia({ video: true }).then(stream => {
          ...
          this.streamCopy = stream;
          this.video.play();

          setTimeout(() => {
            this.capture();
            this.cbCamera(this.canvas.toDataURL());
          }, 1000);
        });
      }
